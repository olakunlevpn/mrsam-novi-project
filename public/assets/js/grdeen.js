(function ($) {
  ...
  if ($(".custom-cursor").length) {
    ...
    // Use event delegation for hover effects so it works on dynamic elements and buttons
    document.addEventListener("mouseover", function (e) {
      if (e.target.closest("a, button, input[type='submit'], input[type='button'], .sort-toggle, .sort-option, .form-check-label, .grdeen-btn, .product__item__cta")) {
  ...
  // Global Fancybox functionality for project images
  $('img').each(function() {
    var $img = $(this);
    var $parentA = $img.closest('a');
    
    if ($parentA.length === 0) {
      $img.wrap('<a href="' + $img.attr('src') + '" class="img-popup global-fancybox" data-group="999" style="display: contents;"></a>');
    } else if (!$parentA.hasClass('img-popup') && !$parentA.hasClass('video-popup')) {
      var href = $parentA.attr('href');
      if (href && (href.match(/\.(jpeg|jpg|gif|png|webp)$/i) !== null)) {
        $parentA.addClass('img-popup global-fancybox').attr('data-group', '999');
      } else if (href === '#' || !href) {
        $parentA.addClass('img-popup global-fancybox').attr('data-group', '999').attr('href', $img.attr('src'));
      }
    }
  });
  ...
  function dynamicCurrentMenuClass(selector) {
    ...
    selector.find("li").each(function () {
      ...
      // Exact match or base filename match (handles ?category=...)
      if (href == FileName || (href && FileName && FileName.startsWith(href + "?"))) {
